...
proc{BindEvents MapTiles RectHandles TrainerH MapID} %Input = {keys,autofight,
   fun{GenerateProc TileId}
      proc{$}
	 if {Send MapID state($)}==ground
	 then {Send TileId switch}
	 else {Show switching}{Send TileId switchTrainer} end
      end
   end
in
   {Show bindingEvents}
   for J in 1..MAXX do
	 ...
	 {TrainerH.J.I bind(event:"<1>" action:{GenerateProc MapTiles.J.I})}
      end
   end
end
...
Map = {ReadMap MapName}
tags(tag:_ handles:RECTHANDLES trainer:TRAINERHANDLES) = {DrawMap MAPCANVAS Map MAXX MAXY}
...
{BindEvents MAPTILES RECTHANDLES TRAINERHANDLES MAPID}